...
FaceDetectorRetina::FaceDetectorRetina(const std::shared_ptr<Ort::Env> &env,
    ...
    if (!FileSystem::fileExists(modelPath)) {
        bool downloadSuccess = Downloader::downloadFileFromURL(m_modelsInfoJson->at("faceAnalyserModels").at("face_detector_retinaface").at("url"),
                                                               "./models");
        if (!downloadSuccess) {
...
void FaceDetectorRetina::preProcess(const VisionFrame &visionFrame, const cv::Size &faceDetectorSize) {
